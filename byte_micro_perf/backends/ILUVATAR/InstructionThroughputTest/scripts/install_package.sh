...
latest_pkg="$(ls -t ${PKG_DIR} | grep ${PKG_NAME} -i | head -1)"
if [[ "${latest_pkg}" == "" ]]; then
...
if [[ "${TARGET_DIR}" != ""  ]]; then
  ...
  echo "ILUVATARKERNEL installed in ${PYTHON_DIST_PATH}; please add it to your PYTHONPATH."
else
